...
- **Artifacts and metrics for every run.** Each run produces a `plan.md`, an `implementation-summary.md`, one `validation-N.md` per round, a per-stage `metrics.json` (wall-clock, files touched, LOC delta), and a `run-summary.json` aggregating those totals plus verdict (`PASS`, `STAGNATED_AFTER_ITERATION_N`, `FAIL_AFTER_MAX_ITERATIONS`, `PLAN_FAILED`, `ABORTED_AT_REVIEW`, etc.), iterations-until-pass, and `.cursor/rules/` bytes injected. `STAGNATED` is the workflow's built-in "review-done" heuristic: when a validation iteration produces the same substantive findings as the prior one, the loop exits early rather than churning to `--max-iterations`. That last part matters more than it sounds: metrics turn "AI is helping, I think" into measured data about where time and code delta actually went — and give you concrete numbers to write about the workflow later.
